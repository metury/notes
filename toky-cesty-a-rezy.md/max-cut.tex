...
\begin{proof}
	...
	$$
	\begin{array}{r c l r}
		\E [W] & =    & \sum_{\{i,j\} \in E} \frac{\theta_{ij}}{\pi} & \text{(by linearity of expectation)} \\
		       & \geq & \alpha \sum_{\{i,j\} \in E} \frac{1 - \cos (\theta_{ij})}{2} & \text{(by the first lemma)} \\
		       & =    & \alpha \sum_{\{i,j\} \in E} \frac{1 - v_{i}^T v_{j}}{2} & \textbf{(this is our objective functions of VP)} \\
		       & \geq & \alpha OPT & \text{(because VP is a relaxation)}
	\end{array}
	$$
\end{proof}