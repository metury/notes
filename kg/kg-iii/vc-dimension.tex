...
Lets see for ourselves an example, which is on picture \ref{broken-x}. For $\mathcal{F}$ we have all half planes in $\R^2$. Then the set $X = \{a,b,c\}$. Then if we draw all lines and choose both planes it will generate $X \cap \mathcal{F} = \{ \textcolor{gray}{\emptyset}, \textcolor{cyan}{\{a\}}, \textcolor{orange}{\{b\}}, \textcolor{teal}{\{c\}}, \textcolor{teal}{\{a,b\}}, \textcolor{cyan}{\{b,c\}}, \textcolor{orange}{\{a,c\}}, \textcolor{gray}{\{a,b,c\}}\}$ which are all subsets of $X$. Hence it breaks the $X$.
