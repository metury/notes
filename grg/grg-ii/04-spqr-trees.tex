...
\begin{proof}
	...
	\begin{proof}
		\begin{enumerate}[I)]
			...
			\noindent Note that if constraint related to cycle vertex position affects me, then it is because of a cycle $\bar{C}$ in $K$ where both edges pf $\bar{C}$ are in $E_{xy}$.
		\end{enumerate}
	\end{proof}
\end{proof}