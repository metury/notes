...
\begin{figure}[!ht]\centering
	\begin{tikzpicture}[node distance={10mm}, main/.style = {draw, thick, circle}]
		...
		\path[->] (2) edge (3') (3') edge (4')
			...
			(6') edge (7');
		% Wings.
		\path[->] (11) edge (10) (10) edge (9) (9) edge (3);
		\path[->] (11') edge (10') (10') edge (9') (9') edge (3');
		% Arms
		\draw[->] (4) edge (12);
		\draw[->] (4') edge (12');
		% Rest of the graph.
		\draw[->] (ph-lower) edge (lower);
		\draw[->] (lower) edge (5);
		\draw[->] (lower) edge (5');
		\draw[->] (upper) edge (ph-upper]);
		%\draw[->] (lower) edge (5);
		%\draw[->] (lower) edge (5');
...
\begin{figure}[!ht]\centering
	...
	\begin{subfigure}{0.45\textwidth}\centering
		\begin{tikzpicture}[node distance={20mm}, main/.style = {draw, thick, circle}]
			...
			\draw[->] (8) edge (11);
			\draw[->] (8') edge (11');
			\draw[dashed] (phantom) edge (mid);
			\draw[dashed] (mid) edge (phantom');
