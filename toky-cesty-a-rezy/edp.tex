...
\begin{figure}[!ht]\centering
	...
	\end{tikzpicture}
	\caption{Graph $H$ as a replacement.}
	\label{h subgraph}
\end{figure}
...
For that we will introduce \textbf{switch} gadget which connects the two previously mentioned gadgets. The exact construction of the switch is shown on the picture \ref{switch}. But it is enough to see the scheme of the switch on the picture \ref{switch scheme}.
...
\begin{figure}[!ht]\centering
	...
	\begin{subfigure}{0.45\textwidth}\centering
		\begin{tikzpicture}[node distance={10mm}, main/.style = {draw, thick, circle}]
			\node[main] (11) {11};
			\node[main, fill, above of = 11] (phantom) {};
			\node[main, above of = phantom] (8) {};
			\node[main, right of = phantom] (mid) {};
			\node[main, fill, right of = mid] (phantom') {};
			\node[main, below of = phantom'] (11') {};
			\node[main, above of = phantom'] (8') {};
			\node[main, above right of = mid] (A) {};
			\node[main, above left of = mid] (B) {};
			\node[main, below right of = mid] (C) {};
			\node[main, below left of = mid] (D) {};
			
			\draw[->] (8) edge (11);
			\draw[->] (8') edge (11');
			\draw (phantom) edge (mid);
			\draw (mid) edge (phantom');
			\draw[->, bend left = 20] (A) edge (mid);
			\draw[->, bend left = 20] (B) edge (mid);
			\draw[->, bend left = 20] (C) edge (mid);
			\draw[->, bend left = 20] (D) edge (mid);
		\end{tikzpicture}
		\caption{}
	\end{subfigure}
...
\begin{enumerate}
	\item If there are 2 edge disjoint paths one leaving at $A$ and the other entering at $B$, then the former is entering at $C$ and the later leaving at $D$.
	\item And exactly one more edge-disjoint path through the graph exists and it is either $8 \to 11$ or $8' \to 11'$.
\end{enumerate}

These switches can be connected by combining $A$ and $C$ and also $B$ and $D$. Therefore we insert the switches so $8 \to 11$ is for the variable in the clause gadget and the other ($8' \to 11'$) is replaced for the variable in the variable clause. After that we arbitrarily connect all switches together. Lastly we also insert vertices $W, X, Y, Z$ as shown on the global picture \ref{global scheme}.
