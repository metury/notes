...
\begin{figure}[!ht]\centering
	\begin{tikzpicture}
		% Set A
		\draw[Blue, fill=Blue!30] (0,0) ellipse (2cm and 1cm);
		\node[Blue] at (0,0) {$A$};
		% Set B
		\draw[Red, fill=Red!30] (2.5,0) ellipse (2cm and 1cm);
		\node[Red] at (2.5,0) {$B$};
		% Intersection
		\begin{scope}
			\clip (0,0) ellipse (2cm and 1cm);
			\clip (2.5,0) ellipse (2cm and 1cm);
			\fill[Green!50] (2.5,0) ellipse (2cm and 1cm);
		\end{scope}
		% Label for intersection
		\node[green] at (1.25,0) {$A \cap B$};
	\end{tikzpicture}
